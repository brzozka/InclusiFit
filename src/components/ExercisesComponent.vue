<template>
    <section class="flex flex-wrap justify-center " role="list">
      <article v-for="exercise in filteredExercises" :key="exercise.name" role="listitem" class="bg-white bg-primary rounded-lg shadow-lg m-4 p-4 w-full md:w-1/2 lg:w-1/3">
        ...
        <div class="flex flex-wrap mt-2 mb-4">
          <span v-for="tag in exercise.tags" :key="tag" class="bg-[#655CFE] bg-yellow text-white text-b text-lg rounded-full px-3 py-1 font-semibold mr-2 mb-2">{{ tag }}</span>
        </div>
