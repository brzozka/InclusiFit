<template>
  <div class="flex flex-col md:flex-row w-full h-full justify-center items-center">
    <img v-if="isImageFirst" :src="imageSrc" :alt="altText" class="md:w-1/2 w-full h-1/2 md:h-full py-2 px-4 rounded-[50px] object-cover" />

    <div class="w-full md:w-1/2 h-full flex flex-col justify-center items-center px-5">
    ...
    <img v-if="!isImageFirst" :src="imageSrc" :alt="altText" class="md:w-1/2 w-full h-1/2 md:h-fit py-2 px-4 rounded-[50px] object-cover" />
  </div>
</template>
