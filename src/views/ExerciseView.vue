<template>
    <main class="relative w-full h-screen">
      ...
      <section aria-labelledby="exercise-title" class="flex flex-col w-full  mx-auto bg-white items-center justify-center flex-grow">
        <h2 id="exercises-title" class="flex flex-col text-4xl font-bold text-[#655CFE] py-20 px-2">
          Discover Exercises Tailored to Your Needs
        </h2>
        <div class="p-4 text-3xl w-full md:w-[70vw]">
          <input type="text" v-model="searchTerm" placeholder="Search by tags..." class="p-2 border border-[#655CFE] rounded-lg w-full " aria-label="Search exercises by tags"/>
          <div class="flex flex-wrap mt-2">
            <button v-for="tag in uniqueTags" :key="tag" @click="filterByTag(tag)" class="bg-[#655CFE] hover:bg-[#201d50] text-white rounded-full px-3 py-1 text-lg font-semibold mr-2 mb-2">
              {{ tag }}
            </button>
          </div>
        </div>
